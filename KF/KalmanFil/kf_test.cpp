...
int main() {
    ...
    int key_y;
    double height_cell;
    std::map<std::pair<int, int>, double> cells_out;
    ...
    for (int i = 0; i < POINTS; i++) {
        int cell_x = ceil(points[i].x / CELL_SIZE);
        int cell_y = ceil(points[i].y / CELL_SIZE);
    ...
    for (int i = 0; i < POINTS; i++) {
        ...
        if (abs(map[cell_x][cell_y].height - cells_out[{cell_x, cell_y}]) > HEIGHT_TOLERANCE) {
            std::cout << "\nWrong height! \n"
                << "CELL(" << cell_x << ")(" << cell_y << ")"
                << " height: " << map[cell_x][cell_y].height
